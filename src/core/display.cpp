...
void drawStatusBar() {
    ...
    // Konumlar STATUS_BAR_HEIGHT'a göre türetilir. Varsayılan 30 -> divider=25,
    // text=12, icon=7 (eski sabit değerlerle birebir aynı). Küçük OLED için
    // STATUS_BAR_HEIGHT board'da küçültülünce status bar otomatik daralır.
    const int textY = (STATUS_BAR_HEIGHT - 18) > 1 ? (STATUS_BAR_HEIGHT - 18) : 2;

    // Durum ikonları 16px yüksekliğinde sabit; STATUS_BAR_HEIGHT küçükse
    // divider çizgisini ikonların altına it ki üst üste binmesinler.
    bool hasStatusIcons = sdcardMounted || gpsConnected || (WiFi.getMode() != 0) || isWebUIActive ||
                          BLEConnected || isConnectedWireguard;
    const int IH_STATUS = 16;
    int dividerY = STATUS_BAR_HEIGHT - 5;
    if (hasStatusIcons && dividerY < IH_STATUS + 2) dividerY = IH_STATUS + 2;

    if (bruceConfig.theme.border) {
        tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, bruceConfig.priColor);
        tft.drawLine(5, dividerY, tftWidth - 6, dividerY, bruceConfig.priColor);
    }

    if (clock_set) {
        setTftDisplay(12, textY, bruceConfig.priColor, 1, bruceConfig.bgColor);
        tft.fillRect(12, textY, 60, LH, bruceConfig.bgColor);
#if defined(HAS_RTC)
    ...
    if (iconCount > 0) {
        ...
        int iy = (STATUS_BAR_HEIGHT - 23) > 0 ? (STATUS_BAR_HEIGHT - 23) : 0; // def 30 -> 7
        int idx = 0;
