...
#ifdef NRF24_2_CE_PIN
    nrf_start2(); // init inject radio (shares SPI bus, separate CE/CS)
#endif
...
#ifdef NRF24_2_CE_PIN
    if (NRFradio2_ready) {
        // Configure Radio2 for injection; Radio1 stays in RX (sniff continues)
        NRFradio2.stopListening();
        NRFradio2.setAutoAck(false);
        NRFradio2.setDataRate(RF24_2MBPS);
        NRFradio2.setPALevel(RF24_PA_MAX);
        NRFradio2.setAddressWidth(5);
        NRFradio2.setChannel(target.channel);
        NRFradio2.setRetries(0, 0);
        NRFradio2.flush_rx();
        NRFradio2.flush_tx();
        NRFradio2.openWritingPipe(addr);
        if (target.type == MJ_DEVICE_MICROSOFT || target.type == MJ_DEVICE_MS_CRYPT)
            NRFradio2.setPayloadSize(19);
        else
            NRFradio2.setPayloadSize(10);
        return; // Radio1 stays untouched
    }
#endif

    // Fallback: single radio, switch Radio1 to TX
