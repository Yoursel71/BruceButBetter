...
static void drawDualJamStatus(NrfJamMode mode, uint8_t ch1, uint8_t ch2, uint8_t strategy,
                               unsigned long elapsedMs, unsigned long sweeps, bool initial) {
    if (initial) { drawMainBorderWithTitle("DUAL JAMMER"); }
    ...
    // Strategy badge
    tft.fillRect(7, y, tftWidth - 14, lh, bruceConfig.bgColor);
    tft.setTextColor(TFT_GREEN, bruceConfig.bgColor);
    tft.drawString(MODE_INFO[(uint8_t)mode].shortName, 12, y + 2, 1);
    const char *stratName = (strategy == 0) ? "SPLIT" : "2xPWR";
    uint16_t bc = (strategy == 0) ? TFT_CYAN : TFT_RED;
    int bw = strlen(stratName) * 6 + 6;
    tft.fillRoundRect(tftWidth - 12 - bw, y, bw, lh - 1, 2, bc);
    tft.setTextColor(TFT_BLACK, bc);
    tft.drawCentreString(stratName, tftWidth - 12 - bw / 2, y + 2, 1);
    y += lh;

    // Radio channels
    tft.fillRect(7, y, tftWidth - 14, lh, bruceConfig.bgColor);
    tft.setTextColor(TFT_YELLOW, bruceConfig.bgColor);
    snprintf(buf, sizeof(buf), "R1:%3d  R2:%3d", ch1, ch2);
    tft.drawCentreString(buf, tftWidth / 2, y + 2, 1);
    y += lh;
    ...
    tft.fillRect(7, y, tftWidth - 14, lh, bruceConfig.bgColor);
    tft.setTextColor(TFT_WHITE, bruceConfig.bgColor);
    unsigned long s = elapsedMs / 1000, m = s / 60; s %= 60;
    snprintf(buf, sizeof(buf), "%02lu:%02lu  S:%lu", m, s, sweeps);
    tft.drawCentreString(buf, tftWidth / 2, y + 2, 1);
    y += lh;

    // Band bar
    {
        int bX = 12, bW = tftWidth - 24, bH = max(5, lh - 3);
        int bY = y + 1;
        tft.fillRect(bX, bY, bW, bH, TFT_DARKGREY);
        // Mark both active channels
        for (int ch : {(int)ch1, (int)ch2}) {
            if (ch < 125) {
                int px = bX + (ch * bW) / 125;
                tft.fillRect(px, bY, 2, bH, (ch == ch1) ? TFT_GREEN : TFT_CYAN);
            }
        }
    ...
    tft.fillRect(7, footY, tftWidth - 14, FP * LH, bruceConfig.bgColor);
    tft.setTextColor(TFT_DARKGREY, bruceConfig.bgColor);
    tft.drawCentreString("[ESC]Stop [<>]Mode", tftWidth / 2, footY, 1);
}

static void runDualJammer(NRF24_MODE nrfMode, NrfJamMode jamMode, uint8_t strategy) {
    NrfJamMode currentMode = jamMode;
    uint8_t ch1 = 0, ch2 = 1;
    unsigned long startMs = millis();
    ...
    // Build channel list
    size_t chCount = 0;
    const uint8_t *chList = getChannelList(currentMode, chCount);
    uint8_t allCh[128];
    size_t total = 0;
    if (currentMode == NRF_JAM_FULL || currentMode == NRF_JAM_DRONE || chCount == 0) {
        for (int i = 0; i < 125; i++) allCh[i] = i;
        total = 125;
    } else {
        memcpy(allCh, chList, chCount);
        total = chCount;
    }

    // Start CW on both radios
    NRFradio.powerUp();  delay(3);
    NRFradio.setPALevel(RF24_PA_MAX);
    NRFradio.startConstCarrier(RF24_PA_MAX, allCh[0]);

    if (NRFradio2_ready) {
        NRFradio2.powerUp(); delay(3);
        NRFradio2.setPALevel(RF24_PA_MAX);
        uint8_t startCh2 = (strategy == 0) ? allCh[min((size_t)1, total-1)] : allCh[0];
        NRFradio2.startConstCarrier(RF24_PA_MAX, startCh2);
    }

    drawDualJamStatus(currentMode, allCh[0], allCh[min((size_t)1,total-1)], strategy, 0, 0, true);

    size_t idx1 = 0, idx2 = (strategy == 0) ? 1 : 0;
    bool running = true;
    ...
    while (running) {
        ...
        if (check(NextPress)) {
            currentMode = (NrfJamMode)(((uint8_t)currentMode + 1) % NRF_JAM_MODE_COUNT);
            chList = getChannelList(currentMode, chCount);
            if (currentMode == NRF_JAM_FULL || currentMode == NRF_JAM_DRONE || chCount == 0) {
                for (int i = 0; i < 125; i++) allCh[i] = i;
                total = 125;
            } else { memcpy(allCh, chList, chCount); total = chCount; }
            idx1 = 0; idx2 = (strategy == 0) ? 1 : 0;
            redraw = true;
        }
        if (check(PrevPress)) {
            currentMode = (NrfJamMode)(((uint8_t)currentMode + NRF_JAM_MODE_COUNT - 1) % NRF_JAM_MODE_COUNT);
        ...
        if (total == 0) { delay(5); continue; }

        // Hop Radio 1
        ch1 = allCh[idx1 % total];
        NRFradio.setChannel(ch1);

        // Hop Radio 2
        if (NRFradio2_ready) {
            if (strategy == 0) {
                // Split: R2 leads by half the list
                ch2 = allCh[(idx1 + total / 2) % total];
            } else {
                // Power: same channel as R1
                ch2 = ch1;
            }
            NRFradio2.setChannel(ch2);
        }

        idx1++;
        if (idx1 >= total) { idx1 = 0; sweeps++; }

        unsigned long now = millis();
        if (now - lastDraw >= 200 || redraw) {
            drawDualJamStatus(currentMode, ch1, ch2, strategy, now - startMs, sweeps, true);
            lastDraw = now;
            redraw = false;
        }
    }
...
#endif // NRF24_2_CE_PIN

// ══════════════════════════════════════════════════════════════════
// ═══════════════ REACTIVE JAMMER ══════════════════════════════
// ══════════════════════════════════════════════════════════════════

static void drawReactiveStatus(const uint8_t *active, uint8_t count, uint8_t curCh,
                                unsigned long elapsed, unsigned long sweeps, bool scanning) {
    drawMainBorderWithTitle(scanning ? "SCANNING..." : "REACTIVE JAM");
    ...
    if (scanning) {
        tft.setTextColor(TFT_YELLOW, bruceConfig.bgColor);
        tft.fillRect(7, y, tftWidth - 14, lh, bruceConfig.bgColor);
        snprintf(buf, sizeof(buf), "Scanning CH %d...", curCh);
        tft.drawCentreString(buf, tftWidth / 2, y + 2, 1);
        y += lh;

        // progress bar
        tft.fillRect(12, y + 2, (curCh * (tftWidth - 24)) / 125, lh - 4, TFT_GREEN);
        tft.drawRect(12, y, tftWidth - 24, lh, bruceConfig.priColor);
        return;
    }
    ...
    tft.setTextColor(TFT_GREEN, bruceConfig.bgColor);
    tft.fillRect(7, y, tftWidth - 14, lh, bruceConfig.bgColor);
    snprintf(buf, sizeof(buf), "Active: %d  CH:%d", count, curCh);
    tft.drawCentreString(buf, tftWidth / 2, y + 2, 1);
    y += lh;
    ...
    tft.setTextColor(TFT_WHITE, bruceConfig.bgColor);
    tft.fillRect(7, y, tftWidth - 14, lh, bruceConfig.bgColor);
    unsigned long s = elapsed / 1000, m = s / 60; s %= 60;
    snprintf(buf, sizeof(buf), "%02lu:%02lu  Sweeps:%lu", m, s, sweeps);
    tft.drawCentreString(buf, tftWidth / 2, y + 2, 1);
    y += lh;
    ...
    // Active channel map
    {
        ...
        for (uint8_t i = 0; i < count; i++) {
            int px = bX + (active[i] * bW) / 125;
            tft.fillRect(px, bY, max(1, bW / 80), bH, TFT_RED);
        }
        // Current position
        if (curCh < 125) {
            int px = bX + (curCh * bW) / 125;
            tft.fillRect(px, bY, 2, bH, TFT_WHITE);
        }
        tft.drawRect(bX, bY, bW, bH, bruceConfig.priColor);
    }
    ...
    tft.fillRect(7, footY, tftWidth - 14, FP * LH, bruceConfig.bgColor);
    tft.setTextColor(TFT_DARKGREY, bruceConfig.bgColor);
    tft.drawCentreString("[ESC]Stop [OK]Rescan", tftWidth / 2, footY, 1);
}

static uint8_t reactiveActive[126];
static uint8_t reactiveCount = 0;

static void scanForActive(uint8_t threshold = 3) {
    uint8_t hits[126] = {0};
    reactiveCount = 0;

    // 3 passes for reliability
    for (int pass = 0; pass < 3; pass++) {
        for (int ch = 0; ch < 125; ch++) {
            ...
            // Update scan display every 5 channels
            if (ch % 5 == 0) {
                drawReactiveStatus(nullptr, 0, ch + pass * 125 / 3, 0, 0, true);
            }
        }
    }

    // Channels seen in 2+ passes are active
    for (int ch = 0; ch < 125; ch++) {
        if (hits[ch] >= threshold) {
            // Add ch and neighbours for better coverage
            for (int d = -1; d <= 1; d++) {
                int nc = ch + d;
                if (nc >= 0 && nc < 125) {
                    // Check not duplicate
                    bool dup = false;
                    for (uint8_t i = 0; i < reactiveCount; i++) {
                        if (reactiveActive[i] == (uint8_t)nc) { dup = true; break; }
                    }
                    if (!dup && reactiveCount < 126) reactiveActive[reactiveCount++] = (uint8_t)nc;
                }
            }
        }
    }

    // If nothing found, fall back to full spectrum
    if (reactiveCount == 0) {
        for (int i = 0; i < 125; i++) reactiveActive[i] = i;
        reactiveCount = 125;
    }
}
...
// ── Quick jam presets (Fast / Heavy) for use from other menus ───────
// Fast  = turbo flood, no dwell, random hop  -> quick, low-footprint
// Heavy = constant carrier (CW), max dwell, max power -> stronger/slower
void nrf_jam_quick(NrfJamMode mode, bool heavy) {
    loadJamConfigs();

    NrfJamConfig backup = jamConfigs[(uint8_t)mode];
    NrfJamConfig &cfg = jamConfigs[(uint8_t)mode];

    if (heavy) {
        cfg.paLevel = 3;     // max power
        cfg.dataRate = 1;
        cfg.dwellTimeMs = 200; // max dwell per channel
        cfg.strategy = 0;    // CW constant carrier
        cfg.randomHop = 0;
    } else {
        cfg.paLevel = 3;
        cfg.dataRate = 1;
        cfg.dwellTimeMs = 0; // turbo
        cfg.strategy = 2;    // turbo flood
        cfg.burstCount = 20;
        cfg.randomHop = 1;
    }

    NRF24_MODE nrfMode = nrf_setMode();
    if (returnToMenu || nrfMode == NRF_MODE_DISABLED) {
        jamConfigs[(uint8_t)mode] = backup;
        return;
    }
    ...
    runJammer(nrfMode, mode);

    jamConfigs[(uint8_t)mode] = backup;
}

void nrf_jam_wifi_fast() { nrf_jam_quick(NRF_JAM_WIFI, false); }
void nrf_jam_wifi_heavy() { nrf_jam_quick(NRF_JAM_WIFI, true); }
void nrf_jam_ble_fast() { nrf_jam_quick(NRF_JAM_BLE_ADV, false); }
void nrf_jam_ble_heavy() { nrf_jam_quick(NRF_JAM_BLE, true); }

// ── Comprehensive jammer presets menu ──────────────────────────────
// Lists every band with Fast + Heavy variants. ~20 options in one place.
void nrf_jammer_presets() {
    // Persistent label storage so .c_str() stays valid through loopOptions
    static std::vector<String> labels;
    labels.clear();
    labels.reserve(NRF_JAM_MODE_COUNT * 2);

    options.clear();
    for (int i = 0; i < NRF_JAM_MODE_COUNT; i++) {
        NrfJamMode m = (NrfJamMode)i;
        labels.push_back(String(MODE_INFO[i].name) + " - Hizli");
        options.push_back({labels.back().c_str(), [m]() { nrf_jam_quick(m, false); }});
        labels.push_back(String(MODE_INFO[i].name) + " - Agir");
        options.push_back({labels.back().c_str(), [m]() { nrf_jam_quick(m, true); }});
    }
    options.push_back({"Single CH", []() { nrf_channel_jammer(); }});
    options.push_back({"CH Hopper", []() { nrf_channel_hopper(); }});
    options.push_back({"Dual Radio Jam", []() { nrf_dual_jammer(); }});
    options.push_back({"Reactive Jam", []() { nrf_reactive_jammer(); }});
    options.push_back({"Advanced (Cfg)", []() { nrf_jammer(); }});
    options.push_back({"Back", [=]() { returnToMenu = true; }});

    loopOptions(options, MENU_TYPE_SUBMENU, "Jammer Presets");
}
...
void nrf_dual_jammer() {
#ifndef NRF24_2_CE_PIN
    displayError("No Radio2 pin!", true);
    return;
#else
    loadJamConfigs();

    uint8_t strategy = 0xFF;
    options.clear();
    options.push_back({"Split Channels", [&]() { strategy = 0; }});
    options.push_back({"Double Power",   [&]() { strategy = 1; }});
    options.push_back({"Back",           [=]() { returnToMenu = true; }});
    loopOptions(options, MENU_TYPE_SUBMENU, "Dual Strategy");
    if (returnToMenu || strategy == 0xFF) return;
    ...
    for (int i = 0; i < NRF_JAM_MODE_COUNT; i++) {
        int mi = i;
        options.push_back({MODE_INFO[i].name, [&selectedMode, &action, mi]() {
                               selectedMode = (NrfJamMode)mi;
                               action = 1;
                           }});
    }
    options.push_back({"Back", [=]() { returnToMenu = true; }});
    loopOptions(options, MENU_TYPE_SUBMENU, "Jam Mode");
    if (returnToMenu || !action) return;

    NRF24_MODE nrfMode = nrf_setMode();
    if (returnToMenu || nrfMode == NRF_MODE_DISABLED) return;
    if (!nrf_start(nrfMode)) { displayError("NRF24 not found"); delay(500); return; }
    if (!CHECK_NRF_SPI(nrfMode)) { displayError("Needs SPI mode"); delay(500); return; }
    if (!nrf_start2()) { displayError("Radio2 not found!\nSingle radio only.", true); }

    runDualJammer(nrfMode, selectedMode, strategy);
#endif
}

void nrf_reactive_jammer() {
    NRF24_MODE nrfMode = nrf_setMode();
    if (returnToMenu || nrfMode == NRF_MODE_DISABLED) return;
    if (!nrf_start(nrfMode)) { displayError("NRF24 not found"); delay(500); return; }
    if (!CHECK_NRF_SPI(nrfMode)) { displayError("Needs SPI mode"); delay(500); return; }
    ...
    NrfJamConfig &cfg = jamConfigs[NRF_JAM_FULL];
    bool flooding = (cfg.strategy >= 1);
    if (flooding) applyJamConfig(cfg, true);
    else          initCW(reactiveActive[0]);
    ...
    while (running) {
        ...
        if (check(SelPress)) {
            if (!flooding) NRFradio.stopConstCarrier();
            scanForActive();
            if (flooding) applyJamConfig(cfg, true);
            else          initCW(reactiveActive[0]);
            hopIdx = 0;
        ...
        if (reactiveCount == 0) { delay(1); continue; }

        uint8_t ch = reactiveActive[hopIdx % reactiveCount];
        switch (cfg.strategy) {
            case 0: cwChannel(ch, cfg.dwellTimeMs); break;
            case 1: floodChannel(ch, cfg.dwellTimeMs, cfg.burstCount); break;
            case 2: turboFloodChannel(ch, cfg.burstCount); break;
        }
        hopIdx++;
        if (hopIdx >= reactiveCount) { hopIdx = 0; sweeps++; }

        unsigned long now = millis();
        if (now - lastDraw >= 250) {
            drawReactiveStatus(reactiveActive, reactiveCount, ch, now - startMs, sweeps, false);
            lastDraw = now;
        }
    }
