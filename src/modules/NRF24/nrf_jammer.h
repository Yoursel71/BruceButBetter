...
/// Quick jam preset: WiFi channels, fast (turbo flood) / heavy (CW max power)
void nrf_jam_wifi_fast();
void nrf_jam_wifi_heavy();

/// Quick jam preset: BLE channels, fast (advertising flood) / heavy (CW data channels)
void nrf_jam_ble_fast();
void nrf_jam_ble_heavy();

/// Comprehensive presets menu: all bands x (Fast/Heavy) + advanced entries
...
/// Dual radio jammer: two NRF24 radios jam simultaneously
/// strategy 0=split (R1 even/R2 odd), 1=power (both same channel)
void nrf_dual_jammer();

/// Reactive jammer: scan active channels first, then jam only those
