...
RF24 NRFradio2(NRF24_2_CE_PIN, NRF24_2_SS_PIN);
bool NRFradio2_ready = false;

bool nrf_start2() {
    if (!NRFSPI) return false; // nrf_start() must be called first to set NRFSPI
    pinMode(NRF24_2_SS_PIN, OUTPUT);
    digitalWrite(NRF24_2_SS_PIN, HIGH);
    NRFradio2_ready = NRFradio2.begin(NRFSPI,
                                       rf24_gpio_pin_t(NRF24_2_CE_PIN),
                                       rf24_gpio_pin_t(NRF24_2_SS_PIN));
    if (NRFradio2_ready) Serial.println("[NRF2] Radio2 OK");
    else                  Serial.println("[NRF2] Radio2 NOT found");
    return NRFradio2_ready;
}
#endif
